...
/**
 * Override of the [Command] interface used for marking a [CelestialBody] as 'hidden'
 *
...
class MarkHidden(
     ...
     * and set its status to 'Hidden'
     */
    override fun execute(): SimpleResult<Long, Multimap<ErrorTag, String>> {
        validateRequest()?.let { return SimpleResult(null, it) } ?: let {
            val theCelestialBody = celestialBodyRepo.findById(id).get()

            theCelestialBody.status = CelestialBody.Status.HIDDEN
    ...
    private fun validateRequest(): Multimap<ErrorTag, String>? {
        ...
        if (!celestialBodyRepo.existsById(id)) {
            ...
            if (theCelestialBody.status == CelestialBody.Status.HIDDEN) {
                errors.put(ErrorTag.STATUS, "Status already marked as 'Hidden'")
            }
        }
